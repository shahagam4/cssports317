...
scatter(answ(:,2),answ(:,3),(answ(:,1)+2.1)*size_factor,'filled','MarkerEdgeColor','k');
hold on;
scatter(answ(:,2),answ(:,3),((answ(:,1)+2.1).*(KineticAndPotential./TotalEnergy))*size_factor,'filled','MarkerEdgeColor','k');
hold on;
scatter(answ(:,2),answ(:,3),((answ(:,1)+2.1).*(Potential./TotalEnergy))*size_factor,'filled','MarkerEdgeColor','k');
hold on;
...
%% Ploting-2
plot(answ(:,2),answ(:,3));   % ploting simple line plot 
hold on;

%% lable the plot
xlabel('y(m)-along length');
ylabel('z(m)-along height');
grid on;
set(gca,'FontSize',20);
